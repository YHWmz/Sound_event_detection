...
class Runner(object):
    ...
    def train(self, config_file, **kwargs):
        ...
        # (file_name, event_label)
        labels_df = pd.read_csv(config['data']['label'],
                                sep='\s+').convert_dtypes()
        # label_array等于labels_df的event_label变成multi-hot向量
        ...
        if config["data"]["specaug"]:
            train_dataset = dataset.TrainDataset_specaug(
                config["data"]["feature"],
                train_df,
                label_to_idx
            )
        elif config["data"]["mix"]:
            train_dataset = dataset.TrainDataset_mix(
                config["data"]["feature"],
                train_df,
                label_to_idx
            )
        elif config["data"]["timeshift"]:
            train_dataset = dataset.TrainDataset_timeshift(
                config["data"]["feature"],
                train_df,
                label_to_idx
            )
        elif config["data"]["freshift"]:
            train_dataset = dataset.TrainDataset_freshift(
                config["data"]["feature"],
                train_df,
                label_to_idx
            )
        elif config["data"]["freaug"]:
            train_dataset = dataset.TrainDataset_freaug(
                config["data"]["feature"],
                train_df,
                label_to_idx
            )
        ...
        if config["sct"]:
            shift_loss_fn = losses.SMseLoss().to(DEVICE)
        ...
        for epoch in range(1, config['epochs'] + 1):
            ...
            with torch.enable_grad(), tqdm(total=len(trainloader), unit="batch", leave=False) as pbar:
                for batch in trainloader:
                    ...
                    loss = loss_fn(output)
                    if config["sct"]:
                        shift_batch, time_shift = self.time_shift_fn(batch)
                        shift_output = self._forward(model, shift_batch)
                        loss_self = shift_loss_fn(output['time_probs'], shift_output['time_probs'], time_shift)
                        loss += 7*loss_self
    ...
    def time_shift_fn(selfself, batch):
        aids, feat, targets = batch
        shift_T = random.randint(10, 100)
        tmp_feat = copy.deepcopy(feat)
        feat = copy.deepcopy(feat)
        zf = random.randint(0, 1)
        if zf:
            feat[:,shift_T:, :] = tmp_feat[:,0:tmp_feat.shape[1] - shift_T, :]
            feat[:,0:shift_T, :] = tmp_feat[:,tmp_feat.shape[1] - shift_T:, :]
        else:
            feat[:,0:feat.shape[1] - shift_T, :] = tmp_feat[:,shift_T:, :]
            feat[:,feat.shape[1] - shift_T:, :] = tmp_feat[:,0:shift_T, :]
            shift_T *= -1
        return (aids, feat, targets), shift_T
            ...
            for batch in tqdm(dataloader, unit='file', leave=False):
                ...
                if postprocessing == 'median':
                    ...
                    if window_size is None:
                        window_size = 1
                    filtered_pred = utils.median_filter(
                        time_prob_batch, window_size=window_size, threshold=thres)
                    time_pred_batch = utils.decode_with_timestamps(
                        idx_to_label, filtered_pred)

                elif postprocessing == 'cATP-SDS':
                    assert time_prob_batch.shape[
                    ...
                    # P(y|x) > alpha
                    clip_pred = utils.binarize(clip_prob_batch, threshold=thres)
                    pred = time_prob_batch * clip_pred
                    ...
                    clip_pred = utils.binarize(clip_prob_batch, threshold=clip_thres)
                    # Apply threshold to
                    pred = clip_pred * time_prob_batch
